...
import cn.darkjrong.mail.domain.EmailAccount;
...
public class EmailUtils {
    ...
    protected static void addTo(Email email, List<EmailTo> toEmails) throws EmailException {
        if (CollectionUtil.isNotEmpty(toEmails)) {
            ...
            Assert.isFalse(index == 0, ErrorEnum.THE_RECEIVER_CANNOT_BE_EMPTY.getMessage());
        }
    }
    ...
    protected static boolean checkFileExists(String filePath) {
        try {
            ...
            log.error(String.format("【%s】, Failed to check file exists, 【%s】", filePath, e.getMessage()), e);
        }
    ...
    private static EmailTemplate getTemplate(EmailAccount emailAccount) {
        ...
        Assert.notNull(emailAccount, ErrorEnum.THE_EMAIL_ACCOUNT_INFORMATION_CANNOT_BE_EMPTY.getMessage());
