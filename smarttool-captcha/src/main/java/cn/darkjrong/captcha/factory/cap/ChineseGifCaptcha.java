...
import cn.darkjrong.captcha.uitls.GifEncoder;
...
public class ChineseGifCaptcha extends AbstractChineseCaptcha {
    ...
    public ChineseGifCaptcha(CaptchaProperties captchaProperties) {
        super(captchaProperties);
    }

    @Override
    public Boolean support(CaptchaType type) {
        return CaptchaType.ChineseGif.equals(type);
    }

    @Override
    protected String toBase64() {
        return "data:image/gif;base64," + Base64.encode(image);
    }
    ...
    protected byte[] drawImg(char[] chars) {
        ...
        try {
            ...
            for (int i = 0; i < len; i++) {
                BufferedImage frame = graphicsImage(fontColor, chars, i, besselXY);
    ...
    private BufferedImage graphicsImage(Color[] fontColor, char[] strs, int flag, int[][] besselXY) {
        ...
        // 画干扰圆圈
        g2d.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.1f * Randoms.num(10)));  // 设置透明度
        drawOval(2, g2d);
        // 画干扰线
